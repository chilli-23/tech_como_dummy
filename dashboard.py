...
# ====================================================================
# ---    PART 1: FUNCTIONS (URL & Database Loading)
# ====================================================================

@st.cache_data
def load_logo_from_url(url):
    """Fetches an image from a public URL."""
    try:
        response = requests.get(url)
        response.raise_for_status()
        return response.content
    except requests.exceptions.RequestException:
        st.warning(f"Could not load logo from URL: {url}")
        return None

@st.cache_resource
def get_engine():
    """Creates a cached database engine using Streamlit Secrets."""
    try:
        ...
        st.warning("Please check your Streamlit Secrets and ensure your database allows connections from Streamlit's IP addresses.")
        return None

# --- EFFICIENT DATA LOADING FUNCTION ---
def load_filtered_data(engine, component, points, equipments):
    """ Loads data from the database, PRE-FILTERED by user selections. """
    if not component or not points or not equipments or engine is None:
        return pd.DataFrame()
    try:
        with engine.connect() as connection:
            query = text("""
                ...
                WHERE d.component = :component
                    ...
                    AND d.equipment_name IN :equipments
                    AND d.value IS NOT NULL
                ORDER BY d.date DESC
            """)
            params = {"component": component, "points": tuple(points), "equipments": tuple(equipments)}
...
if page == "Monitoring Dashboard":
    ...
    def get_all_component_options(_engine):
    ...
    @st.cache_data(ttl=300)
    def get_points_for_component(_engine, component):
        if not component or _engine is None: return []
        try:
            with _engine.connect() as connection:
    ...
    @st.cache_data(ttl=300)
    def get_equipment_for_component(_engine, component):
        if not component or _engine is None: return []
        try:
            with _engine.connect() as connection:
    ...
    with col1:
        component_options = get_all_component_options(engine)
        component_choice = st.selectbox("1. Select Component", options=component_options, index=0 if component_options else None)
    with col2:
        point_options = get_points_for_component(engine, component_choice)
        point_choices = st.multiselect("2. Select Measurement Point(s)", options=point_options)
    with col3:
        equipment_options = get_equipment_for_component(engine, component_choice)
        equipment_choices = st.multiselect("3. Select Equipment(s)", options=equipment_options)
    ...
    if component_choice and point_choices and equipment_choices:
        filtered_df = load_filtered_data(engine, component_choice, point_choices, equipment_choices)
        ...
        plot_df['legend_label'] = plot_df['point_measurement'] + " - " + plot_df['equipment_name']
        ...
        fig = px.line(plot_df, x="date", y="value", color="legend_label", markers=True, title="Trend Comparison", color_discrete_map=color_map)
        fig.update_layout(
            legend_title="Point - Equipment", 
            hovermode="x unified",
...
# ====================================================================
    ...
    if table_to_view:
        ...
        def view_table_data(_engine, table_name):
        ...
        table_df = view_table_data(engine, table_to_view)
